...
colnames(stat.CD11c) <- c("Protein2", "EC Protein" ,"-Log(p.adj)*Direction" )
...
#pdf("E:/Dropbox (UiO)/Validation/SI/DCs/SpatialOmics/IMC/Fig5f.pdf",width=14,height=6)
ggplot(stat.CD11c,aes(x=`EC Protein`,y=`-Log(p.adj)*Direction`,
...
CorAll$Cor.Protein<-as.character(CorAll$Cor.Protein)
CorAll$Cor.Protein<-factor(CorAll$Cor.Protein,levels= c( "ER", "HER2", "CXCL12")) 
...
#pdf("E:/Dropbox (UiO)/Validation/SI/DCs/SpatialOmics/IMC/FigS15b.pdf",width=10,height=4.5)
