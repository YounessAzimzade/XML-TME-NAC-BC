...
fitt$`pCR Score`<-fitt$Slope/(quantile(fitt$Slope,probs=0.95)) 
fitt$`pCR Score`[fitt$`pCR Score`>1] <- 1
fitt$`pCR Score`[fitt$`pCR Score`<  -1] <- -1 

fitt$LI <- fitt$LI*(fitt$`pCR Score`/fitt$Slope) 
fitt$HI <- fitt$HI*(fitt$`pCR Score`/fitt$Slope) 



pdf("~/SHAP values/ER/ER-/Fig3b.pdf",width=16,height=10)
ggplot(fitt)+
...
fittERn <- fitt 
fittERn<-cbind( "ER-", fittERn)
colnames(fittERn) <- c("ER","Cell Type","Slope","LI", "HI", "Sign", "`pCR Score`")