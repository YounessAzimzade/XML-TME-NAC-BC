...
fitAll<-read.csv(".../pCRScoreAll.csv")
fitAll<-cbind("All",fitAll)
colnames(fitAll)<-c("Data","Cell Type","Slope","LI", "HI", "Sign", "pCR Score")
fitAll$`Cell Type`<-as.character(fitAll$`Cell Type`)
fitAll$`Cell Type`<-factor(fitAll$`Cell Type`,levels=fitAll$`Cell Type`)

 

fitPAM<-read.csv(".../pCRScorePAM50.csv")
colnames(fitPAM)<-c("Data","Cell Type","Slope","LI","HI","Sign","pCR Score")
...
fitAllx$Sign[fitAllx$Sign==1] <- "pCR Score > 0"
fitAllx$Sign[fitAllx$Sign==-1] <- "pCR Score < 0"

pdf(".../Fig4e.pdf",width=25,height=9.7)
ggplot(fitAllx,aes(`Cell Type`, forcats::fct_rev(as.factor(Data)),fill=as.factor(Sign),size=abs(`pCR Score`)))+
  ...
  guides(size=guide_legend(override.aes=list(fill = NA,color="black",stroke=.1), 
         ...
         fill = guide_legend(override.aes = list(size = 25),
                             title = NULL))+
  labs(size="Area = |pCR Score|" ,fill="pCR Score",x=NULL,y=NULL)
dev.off()  
